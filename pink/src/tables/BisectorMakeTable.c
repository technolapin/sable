...
/* =============================================================== */
{
  ...
  for (i = 0; i < nval; i++) 
  {
    ...
    for (k = 0; k < n; k++) 
      printf("%d(%d,%d) ", j+k, ListDecs[j+k].x, ListDecs[j+k].y);  
    printf("\n"); 
  } 
} // print_table()
...
/* =============================================================== */
{
  ...
  for (i = 0; i <= nval; i++) 
  {
    n = TabIndDec[i+1] - TabIndDec[i];
    if (n != 0) { k += n; j++; }
    if (i % 100000 == 0) printf("i = %d ; j = %d ; k = %d ; ratio = %g\n", 
...
#ifdef VERBOSE
  print_table(TabIndDec, NVAL, ListDecs, NPOINTS);
#endif
